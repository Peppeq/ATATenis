	...
	<div class="">
		<ol>
			<li v-for="player in AssignedPlayers" :key="player.PlayerId">
				{{ getFullName(player)
				}}<span class="text-danger remove-player" @click="removeTournamentEntry"
					><i :id="player.TournamentEntryId" class="material-icons">clear</i></span
				>
			</li>
		</ol>
		<d-input-group class="mb-2">
			...
			<datalist id="searchedPlayers">
				<option
					v-for="searchedPlayer in searchedPlayers"
					:key="searchedPlayer.PlayerId"
					:value="getFullName(searchedPlayer)"
				/>
				>
			</datalist>
			<d-input-group-addon append>
				<d-btn outline theme="success" :disabled="selectedPlayer == null" @click="addTournamentEntry"
					>Add</d-btn
				>
			</d-input-group-addon>
		</d-input-group>
...
export default class TournamentPlayers extends BaseComponentClass {
	...
	@Prop({ default: null }) readonly RemoveTournamentEntry: (tournamentEntryId: number) => Promise<boolean>;
	@Prop({ default: null }) readonly AddTournamentEntry: (player: PlayerDrawDTO) => Promise<boolean>;
	...
	setSelectedPlayer(event: Event): void {
		...
		const filteredValue = this.searchedPlayers.filter(value => {
			return this.getFullName(value).toUpperCase() === targetValue.toUpperCase();
		})[0];
